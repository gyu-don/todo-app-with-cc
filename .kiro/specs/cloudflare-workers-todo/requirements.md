...
**目的:** システムとして、許可されたユーザーのみがAPIにアクセスできるようにしたい。これにより、不正アクセスを防止し、データのセキュリティを確保する。

#### 受け入れ基準

1. WHEN 認証情報を持たないユーザーがAPIにアクセスした場合、THEN Todoサービスは401 Unauthorizedエラーを返さなければならない
2. WHEN 有効なAPI Keyを含むリクエストを受信した場合、THEN Todoサービスはリクエストを処理しなければならない
3. WHEN 無効または期限切れのAPI Keyでリクエストした場合、THEN Todoサービスは401 Unauthorizedエラーを返さなければならない
4. WHEN Cloudflare Access経由で認証されたユーザーがアクセスした場合、THEN Todoサービスはリクエストを処理しなければならない
5. WHEN 認証ヘッダー（X-API-Key）が欠落している場合、THEN Todoサービスは401 Unauthorizedエラーを返さなければならない

### 要件11: DoS保護とレート制限

**目的:** システムとして、悪意のある大量リクエストから保護し、予期しないコスト発生を防止したい。これにより、サービスの安定性と運用コストの予測可能性を確保する。

#### 受け入れ基準

1. WHEN ボットと判定されるリクエストを受信した場合、THEN Cloudflare Bot Managementはリクエストをブロックしなければならない
2. WHEN 1日の無料枠（100,000リクエスト）を超過した場合、THEN Cloudflare Workersは自動的にサービスを停止し、予期しないコストを防止しなければならない
3. WHILE サービスが稼働している間、THE Todoサービスは悪意のある大量リクエストに対して保護された状態を維持しなければならない
4. IF 将来的にレート制限が必要になった場合、THEN Cloudflare Rate Limiting Rulesを使用して実装できなければならない

### 要件12: データバリデーション

**目的:** システムとして、不正なデータの保存を防止し、データ整合性を維持したい。これにより、セキュリティリスクを軽減し、安定した動作を保証する。

#### 受け入れ基準

1. WHEN Todoタイトルが500文字を超える場合、THEN Todoサービスは400 Bad Requestエラーを返さなければならない
2. WHEN Todoタイトルに制御文字が含まれる場合、THEN Todoサービスは400 Bad Requestエラーを返さなければならない
3. WHEN Todo作成リクエストを受信した場合、THEN Todoサービスは入力データをバリデーションしてから処理しなければならない
4. WHEN 不正なJSON形式のリクエストを受信した場合、THEN Todoサービスは400 Bad Requestエラーを返さなければならない
5. WHEN 完了状態に真偽値以外の値が指定された場合、THEN Todoサービスは400 Bad Requestエラーを返さなければならない

### 要件13: ID生成とデータ構造

**目的:** システムとして、一意で予測不可能なIDを生成し、一貫性のあるデータ構造を維持したい。これにより、セキュリティとデータ整合性を確保する。

#### 受け入れ基準

1. WHEN 新しいTodo項目を作成する場合、THEN TodoサービスはUUID v4形式の一意なIDを生成しなければならない
2. WHEN IDを生成する場合、THEN Todoサービスは暗号学的に安全な乱数生成器を使用しなければならない
3. WHEN Todo項目を保存する場合、THEN Todoサービスは以下のフィールドを含むデータ構造を使用しなければならない：
   - id（string, UUID v4形式）
   - title（string, 最大500文字）
   - completed（boolean）
   - createdAt（ISO 8601形式の日時文字列）
4. WHEN 日時を記録する場合、THEN TodoサービスはISO 8601形式（YYYY-MM-DDTHH:mm:ss.sssZ）を使用しなければならない

### 要件14: エラーレスポンスの標準化

**目的:** クライアントとして、一貫性のあるエラーレスポンス形式を受け取りたい。これにより、エラーハンドリングを統一し、デバッグを容易にする。

#### 受け入れ基準

1. WHEN エラーが発生した場合、THEN Todoサービスは以下の形式のJSONレスポンスを返さなければならない：
   ```json
   {
     "error": {
       "code": "ERROR_CODE",
   ...
   ```
2. WHEN エラーレスポンスを返す場合、THEN Todoサービスは適切なHTTPステータスコードを設定しなければならない
3. WHEN バリデーションエラーが発生した場合、THEN Todoサービスはエラーコード「VALIDATION_ERROR」を使用しなければならない
4. WHEN 認証エラーが発生した場合、THEN Todoサービスはエラーコード「UNAUTHORIZED」を使用しなければならない
5. WHEN リソースが見つからない場合、THEN Todoサービスはエラーコード「NOT_FOUND」を使用しなければならない
6. WHEN 内部エラーが発生した場合、THEN Todoサービスはエラーコード「INTERNAL_ERROR」を使用し、詳細なスタックトレースをログに記録しなければならない（ただし、クライアントには公開しない）
